...
func networkWithRoundObjects(t *testing.T) (*network.Network, types.Round) {
	...
	return network.New(t, cfg), *state.Round
}

func TestShowRound(t *testing.T) {
	net, obj := networkWithRoundObjects(t)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdShowRound(), args)
			if tc.err != nil {
				...
				require.NoError(t, err)
				var resp types.QueryGetRoundResponse
				require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
				require.NotNil(t, resp.Round)
				require.Equal(t,
					nullify.Fill(&tc.obj),
					nullify.Fill(&resp.Round),
				)
			}
		})
	}
